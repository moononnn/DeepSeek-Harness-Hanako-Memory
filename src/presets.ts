...
import { identityTemplate, personaTemplate, renderTemplate, YUAN_KEYS, YUAN_META, type YuanKey } from "./templates.js";
...
export function readSoulConfig(presetPath: string): { config: Record<string, unknown> | undefined; error?: string } {
  const file = join(presetPath, "agent.cordis.yml");
  if (!existsSync(file)) return { config: undefined, error: "缺少 agent.cordis.yml" };
  let parsed: unknown;
  try {
  ...
  if (!Array.isArray(parsed)) return { config: undefined, error: "agent.cordis.yml 不是插件行列表" };
 ...
 */
export function readSoulRows(presetPath: string): {
  rows: unknown[];
  index: number;
  config: Record<string, unknown>;
} {
  const file = join(presetPath, "agent.cordis.yml");
  if (!existsSync(file)) throw new Error("缺少 agent.cordis.yml");
  let parsed: unknown;
  try {
    parsed = yamlLoad(readFileSync(file, "utf8"));
  } catch (e) {
  ...
  if (!Array.isArray(parsed)) throw new Error("agent.cordis.yml 不是插件行列表");
...
export function readPresetMeta(presetPath: string): { name: string; description: string; order?: number } {
...
export function listAgents(presetsRoot: string, defaultId: string | undefined): AgentSummary[] {
  if (!existsSync(presetsRoot)) return [];
  const ids: string[] = [];
  for (const entry of readdirSync(presetsRoot, { withFileTypes: true })) {
    if (entry.isDirectory() && isValidPresetId(entry.name)) ids.push(entry.name);
  }
  return ids
...
/** 读单个助手汇总。 */
export function readAgent(presetsRoot: string, id: string, defaultId: string | undefined): AgentSummary {
  ...
  const base: AgentSummary = {
    ...
    yuan: YUAN_KEYS.includes(config?.yuan as YuanKey) ? (config?.yuan as YuanKey) : "hanako",
...
export function createAgent(
  presetsRoot: string,
  soulRoot: string,
  opts: { name: string; yuan: YuanKey; userName?: string },
): AgentSummary {
  const name = String(opts.name ?? "").trim();
  if (!name) throw new Error("请输入助手名字");
  if (!YUAN_KEYS.includes(opts.yuan)) throw new Error(`未知的元：${opts.yuan}`);
  ...
  if (!existsSync(pinnedFile)) writeFileSync(pinnedFile, "", "utf8");
...
function assertUpdatablePreset(presetsRoot: string, id: string): string {
  if (!isValidPresetId(id)) throw new Error(`非法助手 id：${id}`);
  const dir = join(presetsRoot, id);
  if (!existsSync(dir)) throw new Error(`助手不存在：${id}`);
  return dir;
}
 ...
 */
export function updateAgent(presetsRoot: string, id: string, patch: AgentUpdate, defaultId?: string): AgentSummary {
  const dir = assertUpdatablePreset(presetsRoot, id);

  let read: ReturnType<typeof readSoulRows>;
  try {
  ...
  if (patchKeys.length === 0) throw new Error("没有可更新的字段");
  ...
  if (patch.yuan !== undefined) {
    if (!YUAN_KEYS.includes(patch.yuan)) throw new Error(`未知的元：${patch.yuan}（可选：${YUAN_KEYS.join("/")}）`);
    config.yuan = patch.yuan;
  }

  // 身份 / 人格：原样写入（用户编辑的文本，允许清空）
  if (patch.identity !== undefined) config.identity = String(patch.identity);
  if (patch.persona !== undefined) config.persona = String(patch.persona);

  // 记忆 / 经验开关
  if (patch.memoryEnabled !== undefined) {
    if (typeof patch.memoryEnabled !== "boolean") throw new Error("memoryEnabled 必须是布尔值");
    const memory: Record<string, unknown> = isPlainObject(config.memory) ? { ...config.memory } : {};
    memory.enabled = patch.memoryEnabled;
    config.memory = memory;
  }
  if (patch.experienceEnabled !== undefined) {
    if (typeof patch.experienceEnabled !== "boolean") throw new Error("experienceEnabled 必须是布尔值");
    const experience: Record<string, unknown> = isPlainObject(config.experience) ? { ...config.experience } : {};
    experience.enabled = patch.experienceEnabled;
    config.experience = experience;
  }

  // 原子写回 agent.cordis.yml（保持插件行列表结构，§10.5）
  rows[index] = { ...(rows[index] as Record<string, unknown>), config };
  atomicWrite(join(dir, "agent.cordis.yml"), yamlDump(rows, YAML_DUMP_OPTS));
...
function listAvatarFiles(presetDir: string): string[] {
  const files: string[] = [];
 ...
 */
export function findAvatarFile(presetDir: string): string | undefined {
  return listAvatarFiles(presetDir)[0];
}

/** 校验扩展名是否在白名单内。 */
export function isValidAvatarExt(ext: string): ext is AvatarExt {
  return (AVATAR_EXTS as readonly string[]).includes(ext.toLowerCase());
}
 ...
 * - webp：RIFF....WEBP
 */
export function isValidImageMagic(data: Buffer, ext: string): boolean {
  if (!isValidAvatarExt(ext)) return false;
...
export function writeAvatar(presetsRoot: string, id: string, data: Buffer, ext: string): { file: string; size: number } {
  if (!isValidPresetId(id)) throw new Error(`非法助手 id：${id}`);
  const dir = join(presetsRoot, id);
  if (!existsSync(dir)) throw new Error(`助手不存在：${id}`);
  if (!isValidAvatarExt(ext)) throw new Error(`不支持的图片格式：${ext}（仅支持 png/jpg/jpeg/webp）`);
  if (!isValidImageMagic(data, ext)) throw new Error("文件内容不是有效的图片");
...
export function removeAvatar(presetsRoot: string, id: string): { removed: boolean } {
  if (!isValidPresetId(id)) throw new Error(`非法助手 id：${id}`);
  const dir = join(presetsRoot, id);
  if (!existsSync(dir)) throw new Error(`助手不存在：${id}`);
...
export function avatarFilePath(presetsRoot: string, id: string): string | undefined {
 ...
 */
export function deleteAgent(presetsRoot: string, soulRoot: string, id: string, defaultId?: string): { deleted: string } {
  if (!isValidPresetId(id)) throw new Error(`非法助手 id：${id}`);
  const dir = join(presetsRoot, id);
  if (!existsSync(dir)) throw new Error(`助手不存在：${id}`);
  if (id === defaultId) throw new Error("默认助手不能删除，请先设其他助手为主助手");
  const ids = existsSync(presetsRoot)
    ? readdirSync(presetsRoot, { withFileTypes: true })
        .filter((e) => e.isDirectory() && isValidPresetId(e.name))
        .map((e) => e.name)
    : [];
  if (ids.length <= 1) throw new Error("至少保留一个助手");
 ...
 * 防止前端提交删掉的/伪造的 id。
 */
export function reorderAgents(presetsRoot: string, orderedIds: string[]): AgentSummary[] {
  if (!Array.isArray(orderedIds) || orderedIds.length === 0) throw new Error("排序列表不能为空");
  const current = existsSync(presetsRoot)
    ...
    : new Set<string>();
  if (orderedIds.length !== current.size) throw new Error("排序列表与现有助手数量不一致");
  const seen = new Set<string>();
  for (const id of orderedIds) {
    if (!isValidPresetId(id)) throw new Error(`非法助手 id：${id}`);
    if (!current.has(id)) throw new Error(`排序列表包含不存在的助手：${id}`);
    if (seen.has(id)) throw new Error(`排序列表包含重复 id：${id}`);
    seen.add(id);
  }
