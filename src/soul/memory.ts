...
import {
  MEMORY_FILES,
  formatMemorySnapshotBody as _formatMemorySnapshot,
} from "./compiled-snapshot.js";
...
export function readPinnedEntries(paths: ProfilePaths): PinnedEntry[] {
  try {
...
function writePinnedFiles(paths: ProfilePaths, entries: PinnedEntry[]): void {
...
/** 追加一条置顶记忆（内容去重）。 */
export function addPinnedEntry(paths: ProfilePaths, content: string): { alreadyExists: boolean } {
  const clean = String(content ?? "").trim();
  if (!clean) throw new Error("置顶记忆内容不能为空");
  const entries = readPinnedEntries(paths);
  if (entries.some((entry) => entry.content === clean)) return { alreadyExists: true };
  writePinnedFiles(paths, [...entries, { id: entryId(clean), content: clean }]);
  return { alreadyExists: false };
}

/** 按 id 精确匹配或按内容关键词（包含）删除置顶记忆。 */
export function removePinnedEntry(
  paths: ProfilePaths,
  idOrKeyword: string,
  ...
  if (remaining.length === entries.length) return { removed: false, message: `没有找到匹配「${key}」的置顶记忆` };
...
export function formatPinned(paths: ProfilePaths): string {
  const entries = readPinnedEntries(paths);
...
export {
  readMemorySectionBody as readMemorySection,
  writeCompiledMemorySnapshot as writeCompiledMemory,
  readCompiledMemorySnapshot as readCompiledMemory,
} from "./compiled-snapshot.js";
export type { MemorySectionKey, CompiledMemory } from "./compiled-snapshot.js";

/** 记忆快照渲染文本（systemPrompt variable）；空返回空串，渲染时该段自动消失。 */
export function formatMemorySnapshot(paths: ProfilePaths): string {
  return _formatMemorySnapshot(paths);
}

/* ------------------------------------------------------------------ */
/* 最近对话收集（旧接口保留，供测试与外部调用）                            */
/* ------------------------------------------------------------------ */
...
export function collectRecentConversation(
...
const SECTION_RULES: ReadonlyArray<{ key: "facts" | "today" | "week" | "longterm"; headings: string[] }> = [
 ...
 */
export function parseCompiledMemory(text: string): Partial<Record<"facts" | "today" | "week" | "longterm", string>> | null {
  const lines = String(text ?? "").split(/\r?\n/);
  const heads: Array<{ index: number; heading: string }> = [];
  lines.forEach((line, index) => {
    if (/^##\s+/.test(line)) heads.push({ index, heading: line.replace(/^##\s+/, "").trim() });
  });
  if (heads.length === 0) return null;

  const result: Partial<Record<"facts" | "today" | "week" | "longterm", string>> = {};
  let matched = 0;
...
/* ------------------------------------------------------------------ */
/* 记忆编译 ticker（转发 memory-ticker.ts）                             */
/* ------------------------------------------------------------------ */
