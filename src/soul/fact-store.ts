...
export class FactStore {
  ...
  private readonly tagSearchCache = new Map<string, any>();
  ...
  private _ensureSearchTextColumn(): void {
    try {
      const columns = this.db.prepare("PRAGMA table_info(facts)").all() as Array<{ name: string }>;
  ...
  private _createFtsTriggers(): void {
    try {
      this.db.exec(`
  ...
  /** 新增一条元事实（PII 脱敏后写入）。 */
  add(entry: FactEntry): { id: number } {
    const { cleaned, detected } = scrubPII(String(entry.fact ?? ""));
    if (!cleaned) throw new Error("fact 不能为空");
  ...
  replaceBySession(sessionId: string, entries: FactEntry[]): number {
    ...
    if (!id) throw new Error("replaceBySession requires sessionId");
  ...
  /** 全部（按时间降序）。 */
  getAll(): FactRow[] {
    return (this.stmts.getAll.all() as any[]).map((row) => this._rowToFact(row));
  }
  ...
  getBySession(sessionId: string): FactRow[] {
    return (this.stmts.getBySession.all(sessionId) as any[]).map((row) => this._rowToFact(row));
  }

  /** 删除一个 session 的全部深度记忆事实。 */
  deleteBySession(sessionId: string): number {
    const id = String(sessionId ?? "").trim();
    if (!id) throw new Error("fact invalidation requires sessionId");
   ...
   */
  searchByTags(queryTags: string[], dateRange?: { from?: string; to?: string }, limit = 20): FactRow[] {
    ...
    if (hasFrom) params.dateFrom = dateRange!.from;
    if (hasTo) params.dateTo = dateRange!.to;
    return (stmt.all(params) as any[]).map((row) => this._rowToFact(row));
  }
   ...
   * @param limit - 返回条数上限。
   */
  searchFullText(query: string, limit = 20): FactRow[] {
    if (!query || !query.trim()) return [];
    try {
      const ftsQuery = buildFtsQuery(query);
      if (!ftsQuery) return [];
      if (!this.stmts.ftsSearch) return this._likeFallback(query, limit);
      const rows = this.stmts.ftsSearch.all(ftsQuery, limit) as any[];
  ...
  _likeFallback(query: string, limit = 20): FactRow[] {
    const rows = this.db.prepare("SELECT * FROM facts WHERE fact LIKE '%' || ? || '%' ORDER BY time DESC LIMIT ?").all(query, limit) as any[];
    return rows.map((row) => this._rowToFact(row));
  }

  /** 行 → 对象。 */
  private _rowToFact(row: any): FactRow {
    let tags: string[] = [];
    try {
    ...
    if (row.matchCount !== undefined && row.matchCount !== null) result.matchCount = Number(row.matchCount);
    return result;
  }
