...
export function computeListFingerprint(lines: readonly string[]): string {
  return computeFingerprint(lines.join("\n"));
}

/** 指纹文件路径：`{outputPath}.fingerprint`。 */
export function fingerprintPathFor(outputPath: string): string {
  return `${outputPath}.fingerprint`;
}

/** 读上次指纹；文件缺失返回 null。 */
export function readFingerprint(outputPath: string): string | null {
  try {
 ...
 */
export function shouldCompile(outputPath: string, fingerprint: string): boolean {
  const previous = readFingerprint(outputPath);
  if (previous === null || previous !== fingerprint) return true;
 ...
 */
export function writeFingerprint(outputPath: string, fingerprint: string): void {
