...
/** 关于ta + 人格定义（order 0）。 */
export function buildIdentitySectionText(config: SoulPromptConfig): string {
  const parts: string[] = [`你是 {{assistant_name}}。`];
  if (config.identity.trim()) parts.push(`\n# 关于你（关于ta）\n${config.identity.trim()}`);
  if (config.persona.trim()) parts.push(`\n# 人格定义\n${config.persona.trim()}`);
  return parts.join("\n");
}

/** 记忆 section（order 2）：规则 + 置顶记忆 + 快照。 */
export function buildMemorySectionText(): string {
  return [`# 记忆\n`, MEMORY_RULES, `\n{{pinned_memory}}`, `\n{{memory_snapshot}}`].join("\n");
}

/** 经验 section（order 3）：索引。 */
export function buildExperienceSectionText(): string {
