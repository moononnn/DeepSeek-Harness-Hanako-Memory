...
const TEXT_OUTPUT = (message: string) => [{ type: "text" as const, text: message }];

export function registerTools(ctx: Context, config: SoulConfigFlags, paths: ProfilePaths): void {
  if (config.memory.enabled) {
    ...
    ctx.tools.register(defineTool({
      ...
      async execute(args) {
        ...
        if (!key) throw new Error("unpin_memory 需要提供 id 或 keyword 之一");
