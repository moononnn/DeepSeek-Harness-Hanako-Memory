...
export function normalizeExperienceCategory(raw: string): string {
  const trimmed = String(raw ?? "").trim().replace(/\s+/g, " ");
  if (trimmed.length === 0) throw new Error("分类名不能为空");
  if (trimmed.includes("/") || trimmed.includes("\\") || trimmed.includes("..")) {
    throw new Error("分类名不能包含路径分隔符");
  }
  if (/^[a-zA-Z]:/.test(trimmed)) throw new Error("分类名不能是盘符形式");
  if (/[\x00-\x1f\x7f]/.test(trimmed)) throw new Error("分类名不能包含控制字符");
  return trimmed;
}
 ...
 */
export function buildExperienceStorageFileName(category: string): string {
  const base = category
    .normalize("NFKC")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
...
export function readExperienceDocument(paths: ProfilePaths, category: string): ExperienceDocument | undefined {
...
/** 追加一条经验（去重），然后重建索引。 */
export function recordExperienceEntry(
  paths: ProfilePaths,
  category: string,
  content: string,
  ...
  if (!clean) throw new Error("经验内容不能为空");
 ...
 */
export function rebuildExperienceIndex(paths: ProfilePaths): void {
  let files: string[];
  try {
...
export function readExperienceIndex(paths: ProfilePaths): string {
  try {
