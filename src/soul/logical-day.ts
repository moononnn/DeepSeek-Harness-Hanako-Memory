 ...
 */
export function zonedDateString(nowMs: number, timeZone?: string): string {
 ...
 * @param timeZone - IANA 时区名；缺省用进程本地时区。
 */
export function zonedDayStart(logicalDate: string, timeZone?: string): number {
  const base = parseDateOnly(logicalDate);
  if (!base) throw new Error(`非法逻辑日：${logicalDate}`);
  ...
  if (
    verify.year !== String(base.getFullYear()).padStart(4, "0") ||
...
function wallClockParts(epochMs: number, timeZone?: string): WallClock {
 ...
 * @param timeZone - IANA 时区名；缺省用进程本地时区。
 */
export function getLogicalDay(nowMs: number = Date.now(), timeZone?: string): LogicalDay {
  const formatter = new Intl.DateTimeFormat("en-CA", {
    timeZone: timeZone || undefined,
    year: "numeric",
    month: "2-digit",
...
export function shiftLogicalDate(logicalDate: string, days: number): string {
  const base = parseDateOnly(logicalDate);
  if (!base) return logicalDate;
 ...
 * 非法字符串回落为当前时刻的逻辑日。
 */
export function getLogicalDayFromDate(logicalDate: string, timeZone?: string, nowMs: number = Date.now()): LogicalDay {
  const base = parseDateOnly(logicalDate);
  if (!base) return getLogicalDay(nowMs, timeZone);
  const rangeStart = zonedDayStart(logicalDate, timeZone);
  return { logicalDate, rangeStart, rangeEnd: rangeStart + 86400000 };
}
