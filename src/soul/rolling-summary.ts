...
export function extractMarkdownSection(markdown: string, titles: readonly string[]): string {
  if (!markdown) return "";
  const wanted = new Set(titles.map(normalizeTitle));
  const lines = String(markdown).split(/\r?\n/);
  for (let index = 0; index < lines.length; index += 1) {
    const heading = parseHeading(lines[index]);
    if (!heading || !wanted.has(normalizeTitle(heading.title))) continue;
    const body: string[] = [];
    for (let next = index + 1; next < lines.length; next += 1) {
      const child = parseHeading(lines[next]);
      if (child && child.level <= heading.level) break;
...
export function hasFactSectionHeading(markdown: string): boolean {
  ...
  for (const line of String(markdown).split(/\r?\n/)) {
    ...
    if (heading && titles.has(normalizeTitle(heading.title))) return true;
  }
  return false;
}
...
export function validateRollingSummaryFormat(text: string): { ok: boolean; issues: string[] } {
  const issues: string[] = [];
  const lines = String(text || "").split(/\r?\n/);
  const findHeading = (titles: readonly string[]) => {
    const wanted = new Set(titles.map(normalizeTitle));
    for (let index = 0; index < lines.length; index += 1) {
      const heading = parseHeading(lines[index]);
      if (heading && wanted.has(normalizeTitle(heading.title))) return { index, level: heading.level };
    }
    return null;
  };
  const factHeading = findHeading(FACT_SECTION_TITLES);
  const timelineHeading = findHeading(TIMELINE_SECTION_TITLES);
  if (!factHeading) issues.push('missing fact section heading ("### 重要事实" / "### Key Facts")');
  if (!timelineHeading) issues.push('missing timeline section heading ("### 事情经过" / "### Timeline")');
...
/* ------------------------------------------------------------------ */
/* SessionSummaryStore：summaries/ 目录读写                              */
/* ------------------------------------------------------------------ */
...
export interface SummaryRecord {
  ...
  /** 会话消息时间范围（epoch 毫秒）。 */
  source_time_range: { start: number; end: number } | null;
}
...
export class SessionSummaryStore {
  ...
  constructor(private readonly paths: ProfilePaths) {
  ...
  private mdPath(sessionId: string): string {
  ...
  getSummary(sessionId: string): SummaryRecord | null {
    try {
      ...
      if (!raw || typeof raw !== "object" || typeof raw.session_id !== "string") return null;
      return {
  ...
  getAllSummaries(): SummaryRecord[] {
    return this.listSessionIds()
      ...
      .filter((record): record is SummaryRecord => record !== null && !!record.summary.trim());
  }
 ...
 */
export function deriveSessionMessages(session: Session): SessionMessage[] {
  const timeById = new Map<string, number>();
  for (const event of session.events) {
    if (event.type === "user/message") {
      const data = event.data as { id?: string };
      if (data?.id) timeById.set(data.id, event.time);
    } else if (event.type === "assistant/message") {
      const data = event.data as { message?: { id?: string } };
      if (data?.message?.id) timeById.set(data.message.id, event.time);
    }
  }
  const messages: SessionMessage[] = [];
  for (const message of session.deriveMessages()) {
    if (message.source.kind === "plugin" || message.source.kind === "tool") continue;
...
export function renderConversationText(messages: readonly SessionMessage[]): string {
  return messages
    .map((message) => {
...
export function rollingSummaryBudget(turnCount: number): { totalBudget: number; visibleMaxTokens: number } {
...
/** 滚动摘要生成输入（user 侧）。 */
export function buildRollingSummaryUserPrompt(conversationText: string, previousSummary: string): string {
...
export function readAllSummaryBodies(paths: ProfilePaths): Array<{ sessionId: string; summary: string; updatedAt: string }> {
  const store = new SessionSummaryStore(paths);
...
/** 目录是否存在且非空。 */
export function summariesExist(paths: ProfilePaths): boolean {
  try {
