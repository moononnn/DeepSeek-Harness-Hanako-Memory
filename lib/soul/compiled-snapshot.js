...
export function normalizeCompiledSectionBody(body) {
    return String(body ?? "")
        .split(/\r?\n/)
...
export function normalizeCompiledMemory(value) {
    ...
    if (value && typeof value === "object") {
        for (const { key } of COMPILED_MEMORY_BLOCKS) {
            result[key] = normalizeCompiledSectionBody(typeof value[key] === "string" ? value[key] : "");
        }
    }
    return result;
}
...
/** 读一段已编译记忆；不存在返回空串。 */
export function readMemorySectionBody(paths, key) {
...
/** 读全部四段。 */
export function readCompiledMemorySnapshot(paths) {
    const result = emptyCompiledMemory();
    for (const { key } of COMPILED_MEMORY_BLOCKS)
        result[key] = readMemorySectionBody(paths, key);
    return result;
}
/**
 ...
 * 空串时写空文件（清空该段）。
 */
export function writeMemorySectionBody(paths, key, body) {
    const file = COMPILED_MEMORY_BLOCKS.find((block) => block.key === key)?.file;
    if (!file)
        return;
...
export function writeCompiledMemorySnapshot(paths, compiled) {
    for (const { key } of COMPILED_MEMORY_BLOCKS) {
...
export function parseMemoryMd(content) {
    const result = emptyCompiledMemory();
    const buckets = { facts: [], today: [], week: [], longterm: [] };
    let current = null;
...
/** 读 memory.md；不存在返回空串。 */
export function readMemoryMd(paths) {
    try {
...
export function formatMemorySnapshotBody(paths) {
    const parts = [];
