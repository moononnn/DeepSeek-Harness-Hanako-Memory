...
/** 会话来源时间范围 → 本地日期列表。 */
function localDatesFromRange(range, timeZone) {
    if (!range)
        return [];
...
export function buildFactTimeContext(sourceTimeRange, summaryText, timeZone) {
    const localDates = localDatesFromRange(sourceTimeRange, timeZone);
    const summaryDateTimes = [];
    const summaryDates = [];
    const summaryTimes = [];
    for (const match of String(summaryText).matchAll(DATETIME_PATTERN))
        summaryDateTimes.push(`${match[1]}T${match[2]}:${match[3]}`);
    for (const match of String(summaryText).matchAll(DATE_PATTERN))
        summaryDates.push(match[1]);
    for (const match of String(summaryText).matchAll(TIME_PATTERN))
        summaryTimes.push(`${match[1]}:${match[2]}`);
...
export function buildTimeContextBlock(context, isZh) {
...
export function normalizeFactTime(time, context) {
    if (time == null)
        return null;
    ...
    if (datetime) {
        ...
        if (context.summaryDateTimes.includes(full))
            return full;
        if (context.summaryDates.includes(datetime[1]))
            return full;
        if (context.singleSourceDate === datetime[1])
            return full;
        return null;
    }
    ...
    if (hhmm) {
        ...
        if (!context.summaryTimes.includes(timeKey))
            return null;
        if (context.singleSourceDate && !context.spansMultipleSourceDates)
            return `${context.singleSourceDate}T${timeKey}`;
        return null;
    }
    return null;
}
...
/* ------------------------------------------------------------------ */
function isZh(locale = "zh-CN") {
    return String(locale).startsWith("zh");
}
export function buildFactExtractionSystemPrompt(hasPrevious, locale = "zh-CN") {
    if (isZh(locale)) {
...
export function buildFactExtractionInput(currentSummary, previousSnapshot, timeContext) {
    const zh = isZh();
 ...
 */
export async function processDirtySessions(options) {
    const { ctx, store, factStore, provider, model } = options;
    const timeZone = options.timeZone || "UTC";
    const failCounts = options.failCounts ?? new Map();
    ...
    for (const [sessionId, record] of failCounts) {
        if (nowMs - record.lastUpdated > FAIL_COUNT_TTL_MS)
...
export function listDirtySessions(paths) {
