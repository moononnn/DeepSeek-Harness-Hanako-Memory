 ...
 */
function relabelCall(view, phrase) {
...
function relabelResult(view, phrase) {
    return { ...view, title: phrase };
}
...
const installedCtx = new WeakSet();
 ...
 */
export function registerToolTalk(ctx, opts) {
    if (!opts.enabled)
        return () => { };
    ...
    if (!ctx.tools || typeof ctx.tools.get !== "function")
        return () => { };
    if (installedCtx.has(ctx))
        return () => { };
    installedCtx.add(ctx);
    const fill = (tpl) => tpl.replaceAll(NAME_PLACEHOLDER, opts.name);
        ...
        try {
            for (const [toolName, phrases] of Object.entries(TALK_TABLE)) {
                if (registered.has(toolName))
                    continue;
                const def = ctx.tools.get(toolName, scope);
                if (!def || def[TALK_MARK])
                    continue;
                ...
                ctx.tools.register({
                    ...
                    presentCall: (args) => {
                        ...
                        return view ? relabelCall(view, phrase) : { card: "generic", title: phrase };
                    },
                    presentResult: (args, result) => {
                        const phrase = fill(result.isError ? phrases.failed : phrases.done);
                        const view = origResult ? origResult(args, result) : undefined;
                        return view ? relabelResult(view, phrase) : { card: "generic", title: phrase };
                    },
                });
