...
import { normalizeCompiledSectionBody, readMemorySectionBody, writeMemorySectionBody } from "./compiled-snapshot.js";
/* ------------------------------------------------------------------ */
...
export function buildCompileTodaySystemPrompt(locale = "zh-CN") {
    return isZh(locale)
...
export function buildCompileWeekSystemPrompt(locale = "zh-CN") {
    return isZh(locale)
...
export function buildCompileLongtermSystemPrompt(locale = "zh-CN") {
    return isZh(locale)
...
export function buildCompileFactsSystemPrompt(locale = "zh-CN") {
    return isZh(locale)
...
function summariesInWindow(paths, since, until) {
 ...
 */
export async function compileToday(cx) {
    ...
    if (!shouldCompile(output, fingerprint))
        return "skipped";
 ...
 */
export async function compileWeek(cx) {
    const day = getLogicalDay(cx.now ?? Date.now(), cx.timeZone);
 ...
 */
export async function compileLongterm(cx) {
 ...
 */
export async function compileFacts(cx) {
    const day = getLogicalDay(cx.now ?? Date.now(), cx.timeZone);
    ...
    for (const summary of summaries) {
        ...
        if (factBody && !isEmptyFactSection(factBody))
            candidates.push(factBody);
    }
    if (skipped.length > 0) {
        console.warn(`[assistant-soul] compileFacts: ${skipped.length} 份摘要缺少事实段标题，已跳过: ${skipped.join(", ")}`);
    }
    if (candidates.length === 0)
        return "skipped"; // 无新事实 → 保留旧 facts 原样
