...
export const MEMORY_SECTION_KEYS = ["facts", "today", "week", "longterm"];
/** 读一个 profile 的 memory 目录（只读）；目录不存在返回空快照。 */
export function readMemorySnapshot(soulRoot, profile) {
    const memoryDir = join(soulRoot, profile, "memory");
    const sections = { facts: "", today: "", week: "", longterm: "" };
...
function listSummaries(memoryDir) {
 ...
 */
export function countFacts(dbPath) {
