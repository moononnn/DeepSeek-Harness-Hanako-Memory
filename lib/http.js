...
import { listHanaSources, statHanaSource, matchTarget, importFromHana, defaultHanaAgentsRoot, findSourceAvatar, readHanaConfig } from "./import-hana.js";
...
export function registerRoutes(ctx, config) {
    ...
    ctx.webServer.register({
        ...
        handler: async (req, res) => {
            ...
            try {
                /* ---------- API ---------- */
                /* ---- 从 Hana 转移（本地定制 v0.9.0，核心在 import-hana.js） ---- */
                if (clean === "/api/import/preview" && req.method === "GET") {
                    const agentsRoot = defaultHanaAgentsRoot();
                    const sources = listHanaSources(agentsRoot).map((s) =>
                        Object.assign(s, statHanaSource(agentsRoot, s.id), { target: matchTarget(paths.presetsRoot, s) }));
                    sendJson(res, 200, { sources });
                    return;
                }
                // GET /api/import/avatar?source=<id>：Hana 源助手头像（转移弹窗预览用）
                if (clean === "/api/import/avatar" && req.method === "GET") {
                    const source = String(url.searchParams.get("source") || "");
                    if (!/^[a-z0-9][a-z0-9-]{0,63}$/i.test(source) || !existsSync(join(defaultHanaAgentsRoot(), source, "AGENTS.md"))) {
                        sendJson(res, 404, { error: "源助手不存在" });
                        return;
                    }
                    const av = findSourceAvatar(defaultHanaAgentsRoot(), source);
                    if (!av) {
                        sendJson(res, 404, { error: "该助手没有自定义头像" });
                        return;
                    }
                    const mime = { png: "image/png", jpg: "image/jpeg", jpeg: "image/jpeg", webp: "image/webp" }[av.ext] || "application/octet-stream";
                    res.writeHead(200, { "content-type": mime, "content-length": av.data.length, "cache-control": "no-store" });
                    res.end(av.data);
                    return;
                }
                // POST /api/import：{ sources: [id...], includeMemory, includeExperience }
                if (clean === "/api/import" && req.method === "POST") {
                    ...
                    const ids = Array.isArray(payload.sources)
                        ? payload.sources.filter((v) => typeof v === "string" && /^[a-z0-9][a-z0-9-]{0,63}$/i.test(v))
                        : [];
                    if (ids.length === 0) {
                        sendJson(res, 400, { error: "没有选择要转移的助手" });
                        return;
                    }
                    const includeMemory = Boolean(payload.includeMemory);
                    const includeExperience = Boolean(payload.includeExperience);
                    const agentsRoot = defaultHanaAgentsRoot();
                    const results = [];
                    for (const id of ids) {
                        const srcDir = join(agentsRoot, id);
                        if (!existsSync(join(srcDir, "AGENTS.md"))) {
                            results.push({ source: id, name: id, error: "源助手不存在（缺 AGENTS.md）" });
                            continue;
                        }
                        const cfg = readHanaConfig(srcDir);
                        const source = { id, name: cfg.name || id, yuan: cfg.yuan || "hanako" };
                        try {
                            results.push(importFromHana(paths, agentsRoot, source, includeMemory, includeExperience, currentDefaultId()));
                        }
                        catch (e) {
                            results.push({ source: id, name: source.name, target: null, error: e instanceof Error ? e.message : String(e) });
                        }
                    }
