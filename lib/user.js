 ...
 */
export function readUserYaml(paths) {
    if (!existsSync(paths.userYaml))
        return { name: "", profile: "" };
    let parsed;
    try {
 ...
 * 返回写后的完整数据。
 */
export function writeUserYaml(paths, patch) {
...
export function userAvatarPath(paths) {
    if (!existsSync(paths.userAvatar))
        return undefined;
 ...
 */
export function writeUserAvatar(paths, data) {
...
export function removeUserAvatar(paths) {
