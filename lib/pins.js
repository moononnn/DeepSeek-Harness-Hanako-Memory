...
export function pinnedEntryId(content) {
    return createHash("sha256").update(content).digest("hex").slice(0, 10);
}
/** 一个 profile 的置顶记忆文件路径。 */
export function pinFiles(soulRoot, profile) {
 ...
 * 两者都拿不到时返回空列表。
 */
export function readPinnedEntries(soulRoot, profile) {
...
function writePinnedFiles(soulRoot, profile, entries) {
    const { md, json } = pinFiles(soulRoot, profile);
...
export function addPinnedEntry(soulRoot, profile, content) {
 ...
 */
export function removePinnedEntry(soulRoot, profile, idOrKeyword) {
...
export function pinFilesOnDisk(soulRoot, profile) {
